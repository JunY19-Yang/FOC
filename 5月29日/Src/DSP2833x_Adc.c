...
while (AdcRegs.ADCST.bit.SEQ1_BSY == 1)
     ...
     if(Run_PMSM==2)
     {
          HALL_U=AD_BUF[11]+AD_BUF[10]+AD_BUF[9]+AD_BUF[8];//
          HALL_U=HALL_U>>2;
          
          HALL_V=AD_BUF[15]+AD_BUF[14]+AD_BUF[13]+AD_BUF[12];//
          HALL_V=HALL_V>>2;

     }

    ADC_U=AD_BUF[11]+AD_BUF[10]+AD_BUF[9]+AD_BUF[8];
     ADC_U=ADC_U>>2;

     ADC_V=AD_BUF[15]+AD_BUF[14]+AD_BUF[13]+AD_BUF[12];//
     ADC_V=ADC_V>>2;
//0.732421875=3000mv/4096
//0.010986328125=0.732421875*0.04*1.5  1mv 0.01安 
     ia=_IQdiv(_IQmpy((_IQ(ADC_U)-_IQ(HALL_U)),_IQ(0.0439453125 )),_IQ(E_Ding_DianLiu));//MV
    
     ic=_IQdiv(_IQmpy((_IQ(ADC_V)-_IQ(HALL_V)),_IQ(0.0439453125)),_IQ(E_Ding_DianLiu));//MV
     
     ib=-ic-ia;    // Compute phase-c current
...
void JiSuan_Dl(void)//计算电流
{
    ...
    static Uint16 i=0;
     ...
     IA=(ADC_U-HALL_U)*0.439453125;//放大10倍 
    IC=(ADC_V-HALL_V)*0.439453125;//放大10倍
    IB=0-IA-IC; 
    if(IB<0.0)
    {IB=-IB;}
    if(IA<0.0)
...
void JiSuan_AvgUDC(void)
{
    static Uint16 i=0;
    static Uint32 temp1=0;
    static Uint32 temp2=0;
    float32 j=0;
     temp1+=AD_BUF[6];
      temp2+=AD_BUF[7];
    i++;
      if(i==1000)
{
    ...
    j=temp1;
    temp1=j*1.5*0.732421875/1000.0;
    if(temp1<Udc_zero)
       { temp1=Udc_zero;}
    temp1=temp1-Udc_zero;
    U_dc=1.312*temp1;
